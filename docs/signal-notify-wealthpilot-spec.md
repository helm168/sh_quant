...
**路径** (已落地, 2026-05-26):
- sh_quant `notify_digest.py` 优先读 `~/.market_data/notify_filter.yaml`
  (用户数据, 真实持仓), fallback 仓里 `<sh_quant>/config/notify_filter.yaml`
  (空 portfolio 的模板, 仅供 schema 参考 / WP 研发对照).
- WP 写的目标就是 `~/.market_data/notify_filter.yaml`. 个人持仓属于敏感数据,
  跟 `.env` 同级, **永远不入 git**.
...
- [x] sh_quant `notify_digest.py` 读 `~/.market_data/notify_filter.yaml`
  (fallback `<repo>/config/notify_filter.yaml`). 已落地 2026-05-26.
- [ ] WP 端实现 §4.1-§4.4.
