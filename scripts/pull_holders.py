...
每个 parquet 一只标的、一季一行,按 end_date 升序。含陆股通 A 股个股 +
北向可投资 ETF(均 .SH/.SZ);hk_hold 返回的 HK 行是南向港股通,丢弃。
UI 价值筛选靠 cn_a universe 自然只显示个股,ETF 不上榜。
...
def fetch_quarter(pro, trade_date: str) -> pd.DataFrame:
    ...
    # 保留北向可投资 ETF (ETF331.SZ 等): hk_hold 同 call 返回, 不额外消耗 quota.
    # 价值筛选 UI 用 cn_a 个股 universe 自然过滤掉它们, 不会污染展示; 但 ETF
    # 行在 meta + 磁盘留着, 方便单独研究北向 ETF 流向 (低成本副产物).
